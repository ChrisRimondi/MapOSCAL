...
class TestTemplateBasedGeneration:
    ...
    def test_template_based_generation_success(
    ...
    @patch("maposcal.generator.control_mapper.LLMHandler")
    @patch("maposcal.generator.control_mapper.get_relevant_chunks")
    def test_template_based_generation_with_configuration(
        self, mock_get_chunks, mock_llm_handler_class
    ):
    ...
    @patch("maposcal.generator.control_mapper.LLMHandler")
    @patch("maposcal.generator.control_mapper.get_relevant_chunks")
    def test_template_based_generation_fallback(
        self, mock_get_chunks, mock_llm_handler_class
    ):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for prop in result["props"]:
                if prop["name"] == "control-status":
                    assert (
                        prop["value"] == "applicable and not satisfied"
                    )  # Default fallback

    @patch("maposcal.generator.control_mapper.LLMHandler")
    @patch("maposcal.generator.control_mapper.get_relevant_chunks")
    def test_template_based_generation_with_security_overview(
        self, mock_get_chunks, mock_llm_handler_class
    ):
