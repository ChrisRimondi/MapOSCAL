...
def test_python_identify_imported_modules():
    modules, network, fs, logging_mod, crypto = inspect_lang_python.identify_imported_modules(PYTHON_SAMPLE)
...
def test_python_identify_imported_configuration_variables():
    results = inspect_lang_python.identify_imported_configuration_variables(PYTHON_SAMPLE)
    assert any(r['variable'] == 'API_KEY' and r['method'].startswith('Environment') for r in results)
...
def test_golang_identify_imported_modules():
    modules, network, fs, logging_mod, crypto = inspect_lang_golang.identify_imported_modules(GOLANG_SAMPLE)
    assert 'os' in modules
...
def test_golang_identify_imported_configuration_variables():
    results = inspect_lang_golang.identify_imported_configuration_variables(GOLANG_SAMPLE)
    assert any(r['variable'] == 'apiKey' and r['method'].startswith('Environment') for r in results)
