...
def test_build_faiss_index_empty():
    ...
    with pytest.raises(
        ValueError, match="Cannot build FAISS index with empty vectors array"
    ):
...
def test_search_index_with_invalid_query(sample_index):
    ...
    with pytest.raises(
        AssertionError
    ):  # FAISS raises AssertionError for dimension mismatch
...
def test_index_persistence(sample_vectors, tmp_path):
    ...
    query = sample_vectors[0]
    original_indices, original_distances = faiss_index.search_index(
        original_index, query
    )
